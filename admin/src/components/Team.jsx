  ...
  import NewFunction from "./NewFunction.jsx";
  ...
  const Team = ({ userData, token }) => {
    ...
    useEffect(() => {
      ...
      const fetchReferrals = async () => {
        try {
          const response = await fetch(`${backendUrl}/api/user/referrals/${userId}`);
          ...
          setTotalLeftCC(leftUsers.reduce((sum, u) => sum + (u.cc || 0), 0));
          setTotalRightCC(rightUsers.reduce((sum, u) => sum + (u.cc || 0), 0));
          setInDirectUsers(data.users);
          setTotalIndirectLeft(leftUsers.length);
          setTotalIndirectRight(rightUsers.length);
          setTotalIndirectActive(data.users.filter((u) => u.status === true).length);
          setTotalIndirectDeactive(data.users.filter((u) => u.status === false).length);
    
          // === Hierarchy Construction ===
          const userMap = new Map();
          data.users.forEach((user) => {
            userMap.set(user._id, { ...user, children: [], level: null });
          });
          ...
          data.users.forEach((user) => {
            if (!user.referredBy || !userMap.has(user.referredBy)) {
              const root = userMap.get(user._id);
              if (root) {
                root.level = 1;
                hierarchy.push(root);
              }
    ...
    return (
      <>
        <div>
          ...
          <div className="grid gap-6 sm:grid-cols-1 mt-4 lg:grid-cols-2">
        ...
        {/* <div className="mt-6">
          ...
          >
            <span className="font-semibold">My Generation Team</span>
            <span>
              {isOpen2 ? (
                <i className="fa-solid fa-angle-up"></i>
              ) : (
